...
| 04 Docs experience | ☑ complete (branch `redesign/04-docs-experience`) | _pending_ | Local offline search (MiniSearch); `<Term>` glossary tooltip (hover+focus+keyboard, `role=tooltip`/`aria-describedby`, reduced-motion) + `glossary.ts` (10 entries); static `<SpineDiagram :static>` atop `governance-spine.md` (ASCII fallback kept); "Try this now" golden-path callout in `getting-started.md`; TS/Rust/AS `code-group` tabs (getting-started + first-intent); `sdk/typescript-client.md` rebuilt 26→283 lines (reference-grade). **Refinement:** promoted `.ifx-eyebrow` from `.ifx-home`-scoped to global so the docs callout reuse works. **Extra fix:** purged the flagship `§15.1` leak from governance-spine (no inline exception for §-refs). **Effort-06 handoff (jargon fence):** pre-existing inline-defined type names remain on published pages — `governance-spine.md` (`UnifiedStepParams`, `GetWithActor`, `DisclosureContext`), `getting-started.md` L129 (`TypeOutcomeRecord`), `sdk/typescript-client.md` (`DisclosureContext`, inline-defined). 06's grep must scope to prose/whitelist inline-code-defined types, OR the team purges them. My NEW copy is clean. Fence + build green; Term tooltips verified 6/1/1/4 across pages. |
| 05 Evidence verifier | ☑ complete (branch `redesign/05-evidence-verifier`) | _pending_ | **MECHANISM DEVIATION (user-directed):** shipped the real offline verifier as the first-party **`@infrix/verify`** pure-JS closure (vendored `portableVerifier.js` + `canonicalJson.js` → `theme/lib/verify/`, MIT, from `infrix-sdk-js@db2bfca`), **not** Go→WASM. It's the same independent verifier (byte-exact parity-tested mirror of Go `VerifyPortablePackage`), runs in-browser via Web Crypto, achieves the runbook's exact goal with no toolchain/binary/`wasm_exec.js`. Runbook's WASM path was written not knowing this existed; my monorepo clone was also blocked by `replace ../accumulate_core`. Real fixture `demo-evidence.json` (genuine v4 `PortableEvidencePackage`). `verifier.ts` (lazy dynamic import) + `EvidenceVerifier.vue` (button→spinner→green/amber, aria-live, reduced-motion) wired into walkthrough (mock seam removed). **Proven:** valid fixture → `passed:true`; tampered byte → fails closed (`export_hash`); verifier is a lazy chunk (not in landing); no WASM in dist; landing JS 71.2KB gz (<100KB); green shows real export-hash `0x27d4…54e4`. Fence + build green. **06 handoff:** jargon grep should include `EvidenceVerifier.vue`/`verifier.ts` (copy already clean); optionally assert `theme/lib/verify/` + `demo-evidence.json` exist. |
| 06 Identity & polish | ☑ complete (branch `redesign/06-identity-polish`) | _pending_ | Logo/wordmark system (spine→checkmark `mark.svg`/`lockup.svg`/currentcolor), nav `logo`+`siteTitle`, favicons (svg+32png+apple-touch), 1200×630 OG card (rendered via committed `scripts/render-assets.mjs` + resvg; visually confirmed — real glyphs, spine motif), absolute OG/twitter meta. Global `:focus-visible` default; `<noscript>` reveal fallback (Effort-03 handoff resolved). **Contrast audit fixed 3 real light-theme failures** (`verified`/`pending`/brand-button) via light-only darkened accent variants — all pairings now PASS both themes. **Fence hardened to 5 tests**: theme-files-exist + principled two-tier no-jargon (always-banned `§`/marketing everywhere; API type names banned in prose, allowed in code — resolves Effort-04 handoff without breaking the SDK reference). Motion audit: GPU-only, `--ifx-ease`, reduced-motion all PASS. Full audit recorded in `docs/design/AUDIT.md` — **mechanical checks PASS; axe/Lighthouse/screen-reader/cross-browser/OG-validators honestly marked NEEDS-HUMAN (not runnable headless).** Fence + build green; landing 71.2KB gz. |

**Plan status: all 6 efforts complete.** Remaining before go-live = the NEEDS-HUMAN audit rows in `AUDIT.md` §C (axe, Lighthouse, screen-reader, cross-browser, OG validators) + the release checklist in runbook 06.
